...
| `RESEND_API_KEY` | ✅ prod | API | `re_...` | Email delivery for verification, password reset, invites |
| `RESEND_FROM_EMAIL` | ➖ | API | `SEO Impact OS <onboarding@resend.dev>` | Optional sender override for auth emails |
...
- **Throws in production** if `CRON_SECRET`, `FRONTEND_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, or `RESEND_API_KEY` are missing
...
| Flow | URI Pattern |
|------|------------|
| Better Auth login | `https://<domain>/api/auth/callback/google` |
...
- `RESEND_API_KEY` must be set before signup/password-reset/invite emails work
- `RESEND_FROM_EMAIL` is optional; if omitted, auth mail uses the default Resend sender
